...
class User {
  ...
  static async create({ 
    ...
    try {
      const result = await pool.query(
        `INSERT INTO users (email, password, role, phone, country_id, is_email_verified, email_verification_code, email_verification_expires, subscription_tier)
         VALUES ($1, $2, $3, $4, $5, FALSE, $6, $7, $8) 
         RETURNING id, email, role, is_email_verified, created_at, subscription_tier`,
        [email, password, role, phone, country_id, email_verification_code, email_verification_expires, subscriptionTier]
      );
  ...
  // Set password reset token and expiry (1 hour default)
  ...
  static async findByVerificationCode(token) {
    const result = await pool.query(
      `SELECT id, email, is_email_verified, email_verification_code, 
