...
class Profile {
  ...
  static async updateProfilePicture(userId, pictureUrl) {
    ...
    try {
      ...
      if (result.rows.length === 0) {
        // No existing profile row; inform caller so they can prompt user to complete profile first
        throw new Error('Profile must be created before uploading a profile picture');
      }
   ...
   * @param {number} userId - The ID of the user
   * @returns {object} The user's profile or null if not found
   */
  static async findByUserId(userId) {
    try {
      const query = `
        SELECT p.*, u.email, u.is_email_verified, u.role, u.created_at as user_created_at
        FROM profiles p
        JOIN users u ON p.user_id = u.id
        WHERE p.user_id = $1;
      `;
      ...
      console.error('Error in Profile.findByUserId:', error);
      throw error;
    }
  }
}
