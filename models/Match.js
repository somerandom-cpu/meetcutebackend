...
class Match {
  ...
  static async getUserMatches(userId) {
    const result = await pool.query(
      `SELECT m.*, 
        p1.first_name as user1_first_name, p1.last_name as user1_last_name, COALESCE(p1.profile_picture, p1.profile_pic) as user1_profile_pic,
        p2.first_name as user2_first_name, p2.last_name as user2_last_name, COALESCE(p2.profile_picture, p2.profile_pic) as user2_profile_pic
      FROM matches m
