...
const getProfile = async (req, res) => {
  try {
    ...
    if (!profile) {
      ...
      // For new users, return success with null profile so frontend can proceed
...
const markProfileComplete = async (req, res) => {
  try {
    ...
    // Update users.profile_complete flag via the model method
    const updated = await Profile.markProfileComplete(userId);

    if (!updated) {
      // Even if profile row is missing, consider it success because the user record is updated
      console.warn('Profile row not found, but users.profile_complete updated for user', userId);
    }
