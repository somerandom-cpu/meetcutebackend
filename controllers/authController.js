...
const { sendVerificationEmail, sendVerificationCode, sendEmail } = require('../services/emailService');
...
exports.register = async (req, res) => {
  try {
    ...
    const { email, password, role, phone, countryId, referralCode } = req.body;
    ...
    // Generate 6-digit verification code (string with leading zeros)
    ...
    // If referralCode present, record referral
    if (referralCode) {
      const Referral = require('../models/Referral');
      await Referral.recordReferral(referralCode, user.id);
    }

    // Send code email (no link)
    try {
      const emailResult = await sendVerificationCode(email, email_verification_code);
...
exports.login = async (req, res) => {
  ...
  try {
    ...
    if (!user.is_email_verified) {
      ...
      await pool.query(
        'UPDATE users SET email_verification_code = $1, email_verification_expires = $2, updated_at = CURRENT_TIMESTAMP WHERE id = $3',
...
exports.verifyEmail = async (req, res) => {
  try {
    ...
    // Check token expiry: use updated_at or created_at as issuance time
    const tokenIssuedAt = user.updated_at || user.created_at;
    const expiryHours = parseInt(process.env.EMAIL_VERIFICATION_TOKEN_EXPIRY_HOURS || '24', 10);
    if (tokenIssuedAt) {
...
exports.resendVerification = async (req, res) => {
  try {
    ...
    await pool.query(
      'UPDATE users SET email_verification_code = $1, email_verification_expires = $2, updated_at = CURRENT_TIMESTAMP WHERE id = $3',
      [email_verification_code, codeExpires, user.id]
    );

    // Send code email
    await sendVerificationCode(user.email, email_verification_code);
...
exports.forgotPassword = async (req, res) => {
  try {
    ...
    // Generate 6-digit reset code and expiry (30 min)
    const reset_code = Math.floor(100000 + Math.random() * 900000).toString();
    const expiresAt = new Date(Date.now() + 30 * 60 * 1000);
    await User.setPasswordResetToken(user.id, reset_code, expiresAt);

    // Email the code
    const template = templates.passwordResetCode(reset_code);
    await sendEmail({
      to: user.email,
...
exports.resetPassword = async (req, res) => {
  try {
    ...
    // Clear token
    await User.setPasswordResetToken(user.id, null, null);
