...
// Redirect / to /api/profiles to handle both paths
...
// Get user profile - handle both / and /api/profiles
// The router is already mounted at /api/profiles in server.js, so we only need the root path here.
router.get('/',
  isAuthenticated,
  profileController.getProfile
);

// Serve profile picture through API endpoint
router.get('/picture/:filename', (req, res) => {
  ...
  res.sendFile(filePath, (err) => {
    if (err) {
      ...
      // Send a default avatar if the requested image is not found
...
router.put(
  '/complete',
  isAuthenticated,
  profileController.markProfileComplete
);
